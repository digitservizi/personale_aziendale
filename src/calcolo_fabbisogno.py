...
# ============================================================
# SCRITTURA CONTROPROVA
# ============================================================

def scrivi_controprova(debug_file, anno_analisi, data_esecuzione,
                       calcoli_log, avvisi_log, odc_log, arrot_log):
    """Scrive il file di controprova in formato tabellare leggibile.

    Struttura:
      1. Intestazione con formula e legenda
      2. Sezione per sede: tabella per reparto con tutti i profili
      3. Sezione OdC DM 77
      4. Sezione arrotondamenti (solo casi non banali)
      5. Sezione avvisi figure non censite
    """
    SEP  = '=' * 80
    SEP2 = '─' * 78

    # Costruisce un lookup (sede, reparto, profilo) → entry del calcolo
    # e raggruppa per (sede, reparto) per la stampa tabellare
    per_reparto = defaultdict(list)
    for e in calcoli_log:
        per_reparto[(e['sede'], e['reparto'])].append(e)

    # Raggruppa le righe di arrotondamento per sede
    arrot_per_sede = defaultdict(list)
    for a in arrot_log:
        arrot_per_sede[a['sede']].append(a)
    ...
    with open(debug_file, 'w', encoding='utf-8') as f:
        ...
        f.write(SEP + '\n')
        f.write(
            f'CONTROPROVA CALCOLI FABBISOGNO  ·  Anno di analisi: {anno_analisi}\n'
        )
        f.write(f'Generata il: {data_esecuzione}\n')
        f.write(SEP + '\n\n')
        f.write('Formula applicata:\n')
        f.write(
            '  Fabbisogno = (PL.Omog × T.Occ × Coeff × H.Tur × 365) / H.Ann\n'
        )
        f.write('  dove:\n')
        f.write(
            '    PL.Omog = Ord×1 + DH×0.5 + UTIC×1\n'
        )
        f.write('    T.Occ  = Tasso di Occupazione\n')
        f.write('    Coeff  = Coefficiente di Complessità\n')
        f.write('    H.Tur  = Ore per Turno\n')
        f.write('    H.Ann  = Ore Annue di Lavoro Effettivo\n')
        f.write('\n')

        # ── Sezione calcoli per sede/reparto ─────────────────────────
        f.write(SEP + '\n')
        f.write('1.  CALCOLI PER REPARTO\n')
        f.write(SEP + '\n\n')

        HEADER_FMT = '{:<32} {:>7} {:>6} {:>6} {:>6} {:>7}  {:>9}  {:>6}\n'
        ROW_FMT    = '{:<32} {:>7} {:>6} {:>6} {:>6} {:>7}  {:>9}  {:>6}\n'

        sedi_ordinate = sorted(set(k[0] for k in per_reparto))
        for sede in sedi_ordinate:
            f.write(f'  ┌─  {sede}\n')
            reparti_sede = sorted(
                (k for k in per_reparto if k[0] == sede),
                key=lambda k: k[1],
            )
            for (_, reparto) in reparti_sede:
                entries = per_reparto[(sede, reparto)]
                # Legge intensità e PL.Omog dalla prima entry del reparto
                intensita = entries[0]['intensita'] if entries else '—'
                pl_omg    = entries[0]['pl_omg']    if entries else 0.0
                f.write(
                    f'  │\n'
                    f'  ├──  {reparto}  '
                    f'[Intens.: {intensita}  |  PL omog.: {pl_omg:.2f}]\n'
                )
                f.write(
                    '  │   ' +
                    HEADER_FMT.format(
                        'Profilo', 'T.Occ', 'Coeff',
                        'H.Tur', 'H.Ann', 'PL.Omog', 'F.grezzo', 'F.fin',
                    )
                )
                f.write('  │   ' + SEP2 + '\n')

                # Recupera il valore arrotondato finale dal log
                arrot_map = {
                    a['profilo']: a['f_fin']
                    for a in arrot_per_sede.get(sede, [])
                    if a['reparto'] == reparto
                }
                for e in entries:
                    f_fin = arrot_map.get(e['profilo'], round(e['f_raw']))
                    arrow = ' ◄' if f_fin != round(e['f_raw']) else ''
                    f.write(
                        '  │   ' +
                        ROW_FMT.format(
                            e['profilo'][:32],
        ...
        if odc_log:
            f.write(SEP + '\n')
            f.write('2.  OSPEDALI DI COMUNITÀ  –  fabbisogni da DM 77/2022\n')
            f.write(SEP + '\n\n')
            prev_sede = None
            for o in odc_log:
                if o['sede'] != prev_sede:
                ...
                f.write(
                    f"      {o['reparto']:<45}  "
                    f"{o['profilo']:<35}  →  {o['fabb']}\n"
                )
            f.write('\n')

        # ── Sezione arrotondamenti non banali ─────────────────────────
        if arrot_log:
            f.write(SEP + '\n')
            f.write('3.  ARROTONDAMENTI  (solo casi in cui raw ≠ round standard)\n')
            f.write(SEP + '\n\n')
            ARR_FMT = '  {:<35}  {:<30}  {:<30}  raw={:>8}  →  {:>3}  [{}]\n'
            for a in arrot_log:
                f.write(ARR_FMT.format(
                    a['sede'][:35],
                    a['reparto'][:30],
                    a['profilo'][:30],
                    f"{a['f_raw']:.4f}",
        ...
        if avvisi_log:
            f.write(SEP + '\n')
            f.write('4.  AVVISI  –  figure professionali non censite negli indicatori\n')
            f.write(SEP + '\n\n')
            seen = set()
